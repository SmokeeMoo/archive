...
<form name="update_biolink_" method="post" role="form">
    ...
    <input type="hidden" name="block_type" value="tmquiz" />
    <input type="hidden" name="biolink_block_id" value="<?= $row->biolink_block_id ?>" />
    
    <div class="notification-container"></div>
    
    <!-- Start Items -->
    <button class="btn btn-block btn-gray-300 my-4" type="button" data-toggle="collapse" data-target="#<?= 'tmquiz_items_container_' . $row->biolink_block_id ?>" aria-expanded="false" aria-controls="<?= 'tmquiz_items_container_' . $row->biolink_block_id ?>">
        <?= l('create_biolink_tmquiz_modal.items_header') ?>
    </button>
    
    <div class="collapse" id="<?= 'tmquiz_items_container_' . $row->biolink_block_id ?>">
        
        <div id="<?= 'tmquiz_items_' . $row->biolink_block_id ?>" data-biolink-block-id="<?= $row->biolink_block_id ?>">
            ...
            <div class="mb-4">
                ...
                <div class="d-flex align-items-center "style="border-radius: 6px; padding: 10px; border: 2px solid #d3d3d35e;">	
                    <div class="col-7 col-md-7"> 
                        <div class="d-flex flex-column">
                            <a href="#" data-toggle="collapse" data-target="#<?= 'Question' . ( $key + 1 ) . $row->biolink_block_id ?>" aria-expanded="false" aria-controls="<?= 'Question' . ( $key + 1 )  ?>">
                                <strong><?= !empty($item->name) ? $item->name : l('create_biolink_quiz_modal.market_title.quiz.block') . ' '. ( $key + 1 )  ?></strong>
                            </a>
                    ...
                    <div class="col-2 col-md-2 d-flex justify-content-end">
                        <a href="#" data-toggle="collapse" data-target="#<?= 'Question' . ( $key + 1 ). $row->biolink_block_id  ?>" aria-expanded="false" aria-controls="<?= 'Question' . ( $key + 1 ) . $row->biolink_block_id ?>" >
				...
				<div class="collapse" id="<?= 'Question' . ( $key + 1 ) . $row->biolink_block_id  ?>">	
                    <div class="mb-2">
                        
                        <div class="form-group">
                            <label for="<?= 'item_title_' . $key . '_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-signature fa-sm text-muted mr-1"></i> <?= l('create_biolink_tmquiz_modal.title') ?></label>
                        ...
                        <div class="form-group">
                            <label for="<?= 'item_content1_' . $key . '_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-pen fa-sm text-muted mr-1"></i> <?= l('create_biolink_tmquiz_modal.content1') ?></label>
                            <input id="<?= 'item_content1_' . $key . '_' . $row->biolink_block_id ?>" name="item_content1[<?= $key ?>]" class="form-control" maxlength="2048" required="required" value="<?= $item->content1 ?>" />
                        </div>
                        
                        <div class="form-group">
                            <label for="<?= 'item_content2_' . $key . '_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-pen fa-sm text-muted mr-1"></i> <?= l('create_biolink_tmquiz_modal.content2') ?></label>
                            <input id="<?= 'item_content2_' . $key . '_' . $row->biolink_block_id ?>" name="item_content2[<?= $key ?>]" class="form-control" maxlength="2048" required="required" value="<?= $item->content2 ?>" />
                        </div>
                        
                        <div class="form-group">
                            <label for="<?= 'item_content3_' . $key . '_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-pen fa-sm text-muted mr-1"></i> <?= l('create_biolink_tmquiz_modal.content3') ?></label>
                            <input id="<?= 'item_content3_' . $key . '_' . $row->biolink_block_id ?>" name="item_content3[<?= $key ?>]" class="form-control" maxlength="2048" value="<?= $item->content3 ?>" />
                        </div>
                        
                        <div class="form-group">
                            <label for="<?= 'item_content4_' . $key . '_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-pen fa-sm text-muted mr-1"></i> <?= l('create_biolink_tmquiz_modal.content4') ?></label>
                            <input id="<?= 'item_content4_' . $key . '_' . $row->biolink_block_id ?>" name="item_content4[<?= $key ?>]" class="form-control" maxlength="2048" value="<?= $item->content4 ?>" />
                        </div>
                        
                        <div class="form-group">
                            <label for="<?= 'item_content5_' . $key . '_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-pen fa-sm text-muted mr-1"></i> <?= l('create_biolink_tmquiz_modal.content5') ?></label>
                            <input id="<?= 'item_content5_' . $key . '_' . $row->biolink_block_id ?>" name="item_content5[<?= $key ?>]" class="form-control" maxlength="2048" value="<?= $item->content5 ?>" />
                        </div>
                        
                        <div class="form-group">
                            <label for="<?= 'tmquiz_true_answer_' . $key . '_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-pen fa-sm text-muted mr-1"></i> <?= l('create_biolink_tmquiz_modal.true_answer') ?></label>
                            <input id="<?= 'true_answer_' . $key . '_' . $row->biolink_block_id ?>" name="true_answer[<?= $key ?>]" class="form-control" maxlength="2048" required="required" type="number" min="1" max="5" value="<?= $item->true_answer ?>" />
                        </div>
        ...
        <button data-add="tmquiz_item" data-biolink-block-id="<?= $row->biolink_block_id ?>" type="button" class="btn btn-sm btn-outline-success"><i class="fas fa-fw fa-plus-circle"></i> <?= l('global.create') ?></button>
    </div>
    ...
    <div class="mb-3">
        ...
        <div class="form-group">
            <label for="<?= 'tmquiz_text_color_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-paint-brush fa-sm text-muted mr-1"></i> <?= l('create_biolink_tmquiz_modal.input.text_color') ?></label>
            <input id="<?= 'tmquiz_text_color_' . $row->biolink_block_id ?>" type="hidden" name="text_color" class="form-control" value="<?= $row->settings->text_color ?>" required="required" />
            <div class="text_color_pickr"></div>
        </div>
        
        <div class="form-group">
            <label for="<?= 'tmquiz_background_color_' . $row->biolink_block_id ?>"><i class="fas fa-fw fa-fill fa-sm text-muted mr-1"></i> <?= l('create_biolink_tmquiz_modal.input.background_color') ?></label>
            <input id="<?= 'tmquiz_background_color_' . $row->biolink_block_id ?>" type="hidden" name="background_color" class="form-control" value="<?= $row->settings->background_color ?>" required="required" />
