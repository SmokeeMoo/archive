    ...
    */
    ...
    class GuestPaymentWebhook extends Controller {
        ...
        public function index() {
            ...
            $_GET['processor'] = in_array(($_GET['processor'] ?? null), ['paypal', 'stripe', 'crypto_com', 'razorpay', 'paystack', 'mollie', 'yookassa']) ? input_clean($_GET['processor']) : null;
            ...
            switch($_GET['processor']) {
                ...
                if($notification->getEvent() == NotificationEventType::PAYMENT_SUCCEEDED) {
                    ...
                    /* Start getting the payment details */
                    $payment_id = $notification->getObject()->getId();
                    $payment_total_amount = $notification->getObject()->getAmount()->getValue();
                    $payment_currency = $notification->getObject()->getAmount()->getCurrency();
                    
                    /* Payment payer details */
                    $payer_email = '';
                    $payer_name = l('payment_processors.default_payer_name.yookassa');
                    
                    /* Process meta data */
                    $metadata = $notification->getObject()->getMetadata();
                    $guest_payment_id = $metadata->guest_payment_id;
                }
                break;
