...
.tmnotification__box,
.tmnotification__content,
.tmnotification__btns {
	display: flex;
}
.tmnotification__box,
.tmnotification__content {
...
.tmnotification--out .tmnotification__box {
...
.tmnotification__icon {
	flex-shrink: 0;
	margin-right: 0.75em;
	width: 2em;
	height: 2em;
}
.tmnotification__icon-svg {
	width: 100%;
	height: auto;
}
.tmnotification__text {
	line-height: 1.333;
}
.tmnotification__text-title {
	font-size: 0.75em;
	font-weight: bold;
}
.tmnotification__text-subtitle {
	font-size: 0.6em;
	opacity: 0.75;
}
.tmnotification__btns {
...
.tmnotification__btn-text {
	display: inline-block;
	pointer-events: none;
}
.tmnotification__btn:first-of-type {
	border-radius: 0 0.75rem 0 0;
}
.tmnotification__btn:last-of-type {
	border-radius: 0 0 0.75rem 0;
}
.tmnotification__btn:only-child {
	border-radius: 0 0.75rem 0.75rem 0;
}
.tmnotification__btn + .tmnotification__btn {
	box-shadow: 0 -1px 0 hsla(var(--hue),10%,10%,0.15);
	font-weight: 400;
}
.tmnotification__btn:active,
.tmnotification__btn:focus {
...
@supports selector(:focus-visible) {
	...
	.tmnotification__btn:focus-visible,
	.tmnotification__btn:active {
		background-color: hsl(var(--hue),10%,95%);
	}
}
...
window.addEventListener("DOMContentLoaded",() => {
	const nc = new TMNotificationCenter();
});

class TMNotificationCenter {
	constructor() {
		this.items = [];
		this.itemsToKill = [];
		this.messages = TMNotificationMessages();
...
<?php foreach($data->link->settings->items as $key => $item): ?>
	...
	killNote(id,e) {
		...
		if (note && tar.getAttribute("data-dismiss") === id) {
			note.el.classList.add("tmnotification--out");
...
function TMNotificationMessages() {
	return [
	     <?php foreach($data->link->settings->items as $key => $item): ?>
		{
			icon: "<?= $item->icon ?>",
